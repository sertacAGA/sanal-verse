        ...
        private void Start()
        {
            ...
            if (this.devicesInfoText != null)
            {
                if (UnityMicrophone.devices == null || UnityMicrophone.devices.Length == 0)
                {
                    ...
                    this.devicesInfoText.text = "Mikrofon cihazı bulunamadı!";
                }
                else if (UnityMicrophone.devices.Length == 1)
                {
                    this.devicesInfoText.text = string.Format("Mik.: {0}", UnityMicrophone.devices[0]);
                }
                else
                {
        ...
        private void UpdateUiBasedOnVoiceState(Photon.Realtime.ClientState voiceClientState)
        {
            ...
            switch (voiceClientState)
            {
                case Photon.Realtime.ClientState.Joined:
                    ...
                    this.voiceSwitchText.text = "Sesi Kapat";
                    ...
                    if (this.recorder != null && this.recorder.RecorderInUse != null)
                    {
                        ...
                        this.calibrateText.text = this.recorder.RecorderInUse.VoiceDetectorCalibrating ? "Kalibrasyon" : string.Format("Kalibrasyon ({0}s)", this.calibrationMilliSeconds / 1000);
                    }
                    else
                    {
                        this.calibrateButton.interactable = false;
                        this.calibrateText.text = "Erişilemez";
                    }
                    break;
                case Photon.Realtime.ClientState.PeerCreated:
                case Photon.Realtime.ClientState.Disconnected:
                    ...
                    {
                        this.voiceSwitch.interactable = true;
                        this.voiceSwitchText.text = "Sese Bağlan";
                        this.voiceDebugText.text = String.Empty;
                    }
                    else
                    {
                        this.voiceSwitch.interactable = false;
                        this.voiceSwitchText.text = "Ses N/A";
                        this.voiceDebugText.text = String.Empty;
                    }
                    this.calibrateButton.interactable = false;
                    this.voiceSwitchText.text = "Sese Bağlan";
                ...
                default:
                    this.voiceSwitch.interactable = false;
                    this.voiceSwitchText.text = "Ses Meşgul";
                    break;
            }
        }
